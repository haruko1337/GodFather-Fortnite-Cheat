...
void fn_cache() {
	...
	while (true) {
		...
		if (!offset_uworld)
			offset_uworld = find_signature(E("\x48\x89\x05\x00\x00\x00\x00\x48\x8B\x4B\x78"), E("xxx????xxxx")) - sdk::module_base;
		...
		uint64_t GameInstance = read<uint64_t>(Uworld + 0x190);
		...
		LocalPawn = read<uint64_t>(PlayerController + 0x2A8);
		...
		//printf("LocalRelativeLocation : 0x%llX\n", LocalRelativeLocation);

		uintptr_t GameState = read<uintptr_t>(Uworld + 0x130);
		...
		for (int i = 0; i < Count; i++)
		{
			...
			uint64_t PlayerState = read<uint64_t>(CurrentPawn + 0x238);
			if (!PlayerState)continue;

			uint64_t LocalPlayerState = read<uint64_t>(LocalPawn + 0x238);
...
bool draw_abigsquare()
{
	...
	ImGui::Begin((E("ThePasteFather#0001")), NULL, flags);
	{
		ImGui::PopFont();
...
void render() {
	...
	DrawString(20, 40, 35, &Col.green, true, true, fpsinfo);

	sprintf(fpsinfo, "ThePasteFather - menu key = INSERT", get_fps());
	ImGui::GetOverlayDrawList()->AddText(ImVec2(30, 800), ImGui::GetColorU32({ 199.f, 45.f, 27.f, 4.0f }), fpsinfo);
